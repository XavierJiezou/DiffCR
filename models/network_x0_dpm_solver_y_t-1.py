...
class Network(BaseNetwork):
    ...
    def set_new_noise_schedule(self, device=torch.device('cuda'), phase='train'):
        ...
        # gammas = np.insert(gammas, 0, 1., axis=0)
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None):
        ...
        last_t = t-1
        last_gamma_t1 = extract(self.gammas, last_t-1, x_shape=(1, 1))
        last_sqrt_gamma_t2 = extract(self.gammas, last_t, x_shape=(1, 1))
        last_sample_gammas = (last_sqrt_gamma_t2-last_gamma_t1) * torch.rand((b, 1), device=y_0.device) + last_gamma_t1
        last_sample_gammas = last_sample_gammas.view(b, -1)

        noise = default(noise, lambda: torch.randn_like(y_0))
        y_noisy = self.q_sample(
            y_0=y_0, sample_gammas=sample_gammas.view(-1, 1, 1, 1), noise=noise)
        y_last_noisy = self.q_sample(
            y_0=y_0, sample_gammas=last_sample_gammas.view(-1, 1, 1, 1), noise=noise)
        ...
        if mask is not None:
            ...
            loss = self.loss_fn(y_last_noisy, y_0_hat)
        return loss
