...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.ending = nn.Conv2d(in_channels=width, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                                bias=True)
        self.cond_ending = nn.Conv2d(in_channels=width, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                                bias=True)
        ...
        self.cond_middle_blks = nn.ModuleList()
    ...
    def forward(self, inp, gammas):
        ...
        for encoder, down, cond_encoder, cond_down in zip(self.encoders, self.downs, self.cond_encoders, self.cond_downs):
            ...
            x = x + tmp_cond
            encs.append(x)
            cond_encs.append(cond)
            x = down(x)
            cond = cond_down(cond)

        x = self.middle_blks(x, t)
        cond = self.cond_middle_blks(cond)

        for decoder, up, enc_skip, cond_encoder, cond_up, cond_enc_skip in zip(self.decoders, self.ups, encs[::-1], self.cond_decoders, self.cond_ups, cond_encs[::-1]):
            x = up(x)
            cond = cond_up(cond)
            x = x + enc_skip
            cond = cond + cond_enc_skip
            x = decoder(x, t)
            cond = cond_encoder(cond)
            b, c, h, w = cond.shape
            tmp_cond = cond.view(b//3, 3, c, h, w).sum(dim=1)
...
if __name__ == '__main__':
    ...
    net = UNet()
    flops, params = profile(net, inputs=(torch.Tensor(2, 12, 256, 256), torch.ones(2,)))
