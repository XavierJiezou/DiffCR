...
class CondNAFBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, stride=1, groups=dw_channel,
                               bias=True)
        self.conv3 = nn.Conv2d(in_channels=dw_channel // 4, out_channels=c,
                               kernel_size=1, padding=0, stride=1, groups=1, bias=True)
...
class NAFBlock(EmbedBlock):
    ...
    def forward(self, inp, t, cond):
        ...
        y = y+self.time_emb(t)[..., None, None]+tmp_cond
...
class UNet(nn.Module):
    ...
    def forward(self, inp, gammas):
        ...
        x = self.middle_blks(x, t, cond)

        for decoder, up, enc_skip, cond_skip in zip(self.decoders, self.ups, encs[::-1], cond_encs[::-1]):
            x = up(x)
            x = x + enc_skip
            x = decoder(x, t, cond_skip)
