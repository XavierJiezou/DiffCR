...
class NAFBlock(EmbedBlock):
    def __init__(self, c, DW_Expand=2, FFN_Expand=4, drop_out_rate=0.):
        ...
        self.sg1 = nn.SiLU()
        self.sg2 = nn.SiLU()
        # self.dwconv = nn.Conv2d(in_channels=c, out_channels=c, kernel_size=3, padding=1, stride=1, groups=c)
        ...
        #                        kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=c, kernel_size=3, padding=1, stride=1)
        ...
        self.norm1 = normalization(c)
        self.norm2 = normalization(c)
    ...
    def forward(self, inp, t):
        ...
        # x = x * self.sca(x)
        # x = self.conv3(x)
        x = self.conv1(x) # replace 1

        # x = self.dropout1(x)

        # y = inp + x * self.beta

        y = x+self.time_emb(t)[..., None, None]

        x = self.norm2(y)
        x = self.sg2(x) # replace 2
        x = self.conv2(x)
...
if __name__ == '__main__':
    ...
    flops, params = profile(net, inputs=(
        torch.Tensor(1, 12, 256, 256), torch.ones(1,)))
