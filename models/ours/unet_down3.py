...
class Encoder(nn.Module):
    ...
    def __init__(self, dims=[96, 192, 384, 768], blocks=[1, 1, 1, 1], time_emb_dim=512, dp_rates=0., norm_cfg=None):
        ...
        self.attention = Conv2Former(dims[3], drop_path=dp_rates[-1])

        self.pooling_conv = nn.ModuleList([
            nn.Sequential(nn.Conv2d(dims[0], dims[3], kernel_size=1, stride=1), build_norm_layer(
                norm_cfg, dims[3])[1]),
            nn.Sequential(nn.Conv2d(dims[1], dims[3], kernel_size=1, stride=1), build_norm_layer(
                norm_cfg, dims[3])[1]),
            nn.Sequential(nn.Conv2d(dims[2], dims[3], kernel_size=1, stride=1), build_norm_layer(
                norm_cfg, dims[3])[1]),
        ])
    ...
    def forward(self, x, t):
        ...
        global_f = F.adaptive_avg_pool2d(self.pooling_conv[0](c3), output_size=(c6.shape[-2], c6.shape[-1])) \
            + F.adaptive_avg_pool2d(self.pooling_conv[1](
                c4), output_size=(c6.shape[-2], c6.shape[-1])) + F.adaptive_avg_pool2d(self.pooling_conv[2](c5), output_size=(c6.shape[-2], c6.shape[-1]))+c6
        global_f = self.attention(global_f)
        return c3, c4, c5, c6, global_f
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        c3, c4, c5, c6, global_f = x
        c6 = self.decoder_conv[0](c6, global_f)
        c5 = self.decoder_conv[1](c5, global_f)
        c4 = self.decoder_conv[2](c4, global_f)
        c3 = self.decoder_conv[3](c3, global_f)

        c5 = self.decoder_up[0](c5, c6)
        c4 = self.decoder_up[1](c4, c5)
        c3 = self.decoder_up[2](c3, c4)
        return c3, c4, c5, c6
